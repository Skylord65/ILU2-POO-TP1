...
public class Village {
	...
	private class Marche {
		...
		private Etal[] trouverEtals(String produit) {
			...
			for (int i = 0, valeurAjoutee = 0; i < etals.length; i++) {
				if (etals[i].contientProduit(produit)) {
		...
		private Etal trouverVendeur(Gaulois gaulois) {
			for (int i = 0; i < etals.length; i++) {
				if (etals[i] != null && etals[i].getVendeur().equals(gaulois)) {
					return etals[i];
				}
			}
			return null;
	...
	public Gaulois trouverHabitant(String nomGaulois) {
		...
		for (int i = 0; i < nbVillageois; i++) {
			...
			if (gaulois != null && gaulois.getNom().equals(nomGaulois)) {
				return gaulois;
			}
		}
		return null;
	...
	public String rechercherVendeursProduit(String produit) {
		...
		if (listeEtal.length == 0) {
			...
			chaine.append("Seul le vendeur " + listeEtal[0].getVendeur().getNom() + " propose des " + produit
					+ " au marché.\n");
		} else {
